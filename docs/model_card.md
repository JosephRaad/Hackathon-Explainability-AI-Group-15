...
- **Post-processing**: Group-specific threshold equalization when Reweighing alone is insufficient
...
For attributes with >2 groups (RaceDesc has 7), we binarize to majority-vs-rest before applying Reweighing. This ensures all samples receive adjusted weights, not just the top 2 groups.
